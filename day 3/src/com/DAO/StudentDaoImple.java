...
/*== Second : This Is for Student object:We can use this method or above method for registration of student ==*/
	@Override
...
////======== 7th Step:
@Override
public List<StudentDTO> getAllStudentsByCname(String courseName) throws CourseException {

	List<StudentDTO> dtos = new ArrayList<>();

	try (Connection conn = DButil.provideConnection()) {

		PreparedStatement ps = conn.prepareStatement("select s.stdID, s.Stdname,s.mobile,s.email, c.courseName, c.coursefee "
				+ "from  student s INNER JOIN course c INNER JOIN course_student cs "
				+ "ON s.stdID = cs.stdID AND c.courseId = cs.courseId AND c.courseId= ?");
		...
		while (rs.next()) {
			...
			StudentDTO dto = new StudentDTO(r, sn,sm, em, cn, f);
