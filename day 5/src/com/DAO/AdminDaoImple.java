...
public List<StudentDTO> viewAllStudent(int stdID, int courseid) throws BranchException {
   List<StudentDTO> s1=null;
	
	try (Connection conn=DButil.provideConnection()){
		
//PreparedStatement ps= conn.prepareStatement("select s.stdid, s.stdname, s.email,s.mobile, "
//	+ "m."+CourseName+" from student s, "+BranchName+" m where s.stdid=m.stdid");
//		ps.setString(1, CourseName);
//		ps.setString(2, BranchName);
		
PreparedStatement ps=conn.prepareStatement("select s.stdID,s.stdName,s.mobile,s.email,c.courseId,c.CourseName,c.coursefee ,c.duration from student s,course c,studentdto sc where s.stdID=sc.stdID AND c.courseId=sc.courseId");				
		ResultSet rs= ps.executeQuery();
		...
		if(rs.next()) {
			...
			String cfee=rs.getString("coursefee");
			String cdur=rs.getString("duration");
			
			StudentDTO dto=new StudentDTO(id, name,  mobile,email,cid,cname,cfee,cdur);
			s1.add(dto);
